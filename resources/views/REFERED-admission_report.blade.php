                ...
                <div class="card">
                    <div class="card-body">
                        <div class="form-validation">
                            <form class="form-valide" action="{{ $url }}" method="post" target="_blank">
                                @csrf
                                <fieldset class="required form-label">Admission by REFERED Report</fieldset>
                                <legend class="form-control">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="exampleFormControlInput1" class="required form-label">REFERED Name</label>
                                            <div class="form-floating input-group input-group-solid mb-5">
                                                <select class="form-select all_sum" id="dr_id" name="dr_id"
                                                    aria-label="Floating label select example">
                                                    <option value="" selected>Open this select menu</option>
                                                    @foreach ($PatReferedBy as $val)
                                                        <option value="{{ $val->pat_id }}">
                                                            {{ $val->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        {{-- <div class="form-group col-md-4">
                                            ...
                                            <div class="input-group input-group-solid mb-5">
                                                <input type="text" name="share" class="form-control " id="share"
                                                    aria-label="Amount (to the nearest dollar)" value="{{ old('share') }}"
                                                    min="0" maxlength="11" />
                                            </div>
                                            <span class="text-danger">
                                                @error('share')
